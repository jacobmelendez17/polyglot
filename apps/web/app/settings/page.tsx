...
// Settings (spec §16). Every option auto-saves on change — no "save" button to
// forget. A field that fails server validation surfaces its error inline and the
// control reverts. The immersion toggle is disabled until it's unlocked (level 10).
// Loading / error states throughout; controls are labelled for screen readers.
...
import { applyAppearance, COLOR_THEMES } from "@/lib/appearance";
...
function SettingsForm() {
  ...
  async function save(field: keyof Settings, value: unknown) {
    ...
    setFieldError((e) => ({ ...e, [field as string]: "" }));
    try {
      const updated = await account.updateSettings({ [field]: value });
      setS(updated);
      if (field === "theme" || field === "font_size" || field === "color_theme") {
        applyAppearance({ theme: updated.theme, font_size: updated.font_size,
      ...
      setS(prev);                                         // revert
      const fe = (err as { field_errors?: Record<string, string> })?.field_errors
    ...
    <div className="flex flex-col gap-6">
      ...
      <Section title="appearance">
        <Select label="theme" value={s.theme} err={fieldError.theme}
          options={["light", "dark", "system"]} onChange={(v) => save("theme", v)} saved={saved === "theme"} />
        <Select label="font size" value={s.font_size} err={fieldError.font_size}
          options={["sm", "md", "lg", "xl"]} onChange={(v) => save("font_size", v)} saved={saved === "font_size"} />
        <Select label="color theme" value={s.color_theme} err={fieldError.color_theme}
          options={[...COLOR_THEMES]} onChange={(v) => save("color_theme", v)}
          saved={saved === "color_theme"} />
      </Section>

      <Section title="lessons">
        <Num label="batch size" value={s.lesson_batch_size} min={1} max={50} err={fieldError.lesson_batch_size}
          onChange={(v) => save("lesson_batch_size", v)} saved={saved === "lesson_batch_size"} />
      </Section>
      ...
      <Section title="reviews">
        ...
        <Num label="batch size" value={s.review_batch_size} min={1} max={100} err={fieldError.review_batch_size}
          onChange={(v) => save("review_batch_size", v)} saved={saved === "review_batch_size"} />
        ...
        <Num label="leech threshold" value={s.leech_threshold} min={0.1} max={5} step={0.1}
          err={fieldError.leech_threshold}
          onChange={(v) => save("leech_threshold", v)} saved={saved === "leech_threshold"} />
      </Section>
      ...
      <Section title="extras">
        <Toggle label="intermissions" value={s.intermissions_enabled}
  ...
  return (
    <Row label={label} saved={saved}>
      <button role="switch" aria-checked={value} aria-label={label} disabled={disabled}
        ...
        className={`h-6 w-11 rounded-full transition-colors disabled:opacity-40 ${value ? "bg-terraza-green" : "bg-terraza-dash"}`}>
...
function Select({ label, value, options, onChange, saved, err }: {
  label: string; value: string; options: string[]; onChange: (v: string) => void; saved?: boolean; err?: string;
}) {
  return (
    <div>
      <Row label={label} saved={saved}>
        <select value={value} aria-label={label} onChange={(e) => onChange(e.target.value)}
          className="rounded-card border border-terraza-dash bg-terraza-bg px-2 py-1 text-sm">
          {options.map((o) => <option key={o} value={o}>{o.replace(/_/g, " ")}</option>)}
        </select>
      </Row>
      {err && <p className="text-xs text-terraza-danger">{err}</p>}
    </div>
  );
}

function Num({ label, value, min, max, step, onChange, saved, err }: {
  ...
  return (
    <div>
      <Row label={label} saved={saved}>
        <input type="number" value={value} min={min} max={max} step={step ?? 1} aria-label={label}
          onChange={(e) => onChange(step ? parseFloat(e.target.value) : parseInt(e.target.value, 10))}
          className="w-20 rounded-card border border-terraza-dash bg-terraza-bg px-2 py-1 text-sm" />
      </Row>
      {err && <p className="text-xs text-terraza-danger">{err}</p>}
    </div>
  );
...
function Text({ label, value, onChange, saved, err }: {
  label: string; value: string; onChange: (v: string) => void; saved?: boolean; err?: string;
}) {
  ...
  return (
    <div>
      <Row label={label} saved={saved}>
        <input value={local} aria-label={label} onChange={(e) => setLocal(e.target.value)}
          onBlur={() => local !== value && onChange(local)}
          className="w-32 rounded-card border border-terraza-dash bg-terraza-bg px-2 py-1 text-sm" />
      </Row>
      {err && <p className="text-xs text-terraza-danger">{err}</p>}
    </div>
  );
